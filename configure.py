...
if IsInChina == 0:
    ...
    data.update({"address": geo["formattedAddress"], "area": geo["addressComponent"]["province"] + ' ' + geo["addressComponent"]["city"] +
                 ' ' + geo["addressComponent"]["district"], "province": geo["addressComponent"]["province"], "city": geo["addressComponent"]["city"]})
...
# 体温范围
data.update({"tw": AskInteractive("今日体温范围(℃): ", ["(-inf, 36]", "(36, 36.5]", "(36.5, 36.9]",
                                                  "(36.9, 37.3]", "(37.3, 38]", "(38, 38.5]", "(38.5, 39]", "(39, 40]", "(40, +inf)"], 1)})
...
data.update({"sfcxtz": AskBoolean("今日是否出现发热、乏力、干咳、呼吸困难等症状: "),
             "sfyyjc": 0, "jcjgqr": 0, "jcjg": ""})
...
# 检查结果属于以下哪种情况
if data["sfyyjc"] == 1:
    data.update({"jcjgqr": AskInteractive(
        "检查结果属于以下哪种情况: ", ["疑似感染", "确诊感染", "其他"], 1)})
...
# 今日是否接触无症状感染/疑似/确诊人群
data.update({"sfjcbh": AskBoolean("今日是否接触无症状感染/疑似/确诊人群: "),
             "jcbhlx": "", "jcbhrq": ""})
...
if data["sfcyglq"] == 1:
    # 隔离场所
    iso_list = ["校内居家隔离", "校内医院隔离", "校外居家隔离",
                "校外西安市内医学隔离", "校外西安市内集中隔离", "西安市外地区医学隔离"]
